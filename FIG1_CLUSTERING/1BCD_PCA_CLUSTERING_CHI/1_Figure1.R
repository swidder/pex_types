...
#SUPP FIGURE
if (i==5){
nm<-paste("SUPP_2A_chi2_",colnames(as.data.frame(tB))[1],"_",colnames(as.data.frame(tB))[2],".pdf",sep="")
...
p4<-ggplot(dp, aes(x=k,y=e))+geom_line()+geom_point(size=2, shape=21, fill='white')+meg_stef_theme()+ylab(expression(chi^{2}))+xlab("K Clusters")+ggtitle("Exacerbation grouping")
