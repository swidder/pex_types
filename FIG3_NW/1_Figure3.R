...
pA<-ggplot(tmp, aes(x=K3, y=ecount, color=K3))+geom_boxplot(outlier.shape=NA)+geom_jitter(shape=16, position=position_jitter(0.2), size=0.5,alpha=0.5)+stat_compare_means(comparisons=comp,label = "p.signif", size=4.2, label.y=c(3,3.6,4.6),method.args = list(alternative = "two.sided"))+xlab("PEx types")+ylab("Edges (scaled)")+ meg_stef_theme() + guides(color=guide_legend(override.aes=list(fill=NA)))+ scale_color_manual(values=cfcol,name="PEx types",breaks=c( 'PAT','AN1', 'AN2'))+ylim(c(-1.5,5.5))+ggtitle("Node pairing")
#stat_compare_means(comparisons=comp,label = "p.format", size=3.5, label.y=c(3,3.6,4.2))

pB<-ggplot(tmp, aes(x=K3, y=vcount, color=K3))+geom_boxplot(outlier.shape=NA)+geom_jitter(shape=16, position=position_jitter(0.2), size=0.5,alpha=0.5)+stat_compare_means(comparisons=comp,label = "p.format", size=4.2, label.y=c(3,3.6,4.6))+xlab("PEx types")+ylab("Nodes (scaled)")+ meg_stef_theme() + guides(color=guide_legend(override.aes=list(fill=NA)))+ scale_color_manual(values=cfcol,name="PEx types",breaks=c( 'PAT','AN1', 'AN2'))+ylim(c(-1.5,5.5))+ggtitle("Size")
...
########
#DEGREE DISTRIBUTION P(k), CLUSTERING DISTRIBUTION (P(CCi))
#FIG3DE
#######
...
################
#MASTER ORGANISMS IN HIERARCHY (giant component)
#MIN 5% APPEARANCES
#FIG3F
###########
...
#probability of top hierarchy position in NW
top<-table(me$ASV)
