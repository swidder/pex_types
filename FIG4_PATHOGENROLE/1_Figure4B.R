...
dfC<-data.frame(met[,c(22,3,19,21,8)])
dfC<-get_org_color_df(dfC, S, NTT, tax, "C")
dfN<-data.frame(met[,c(22,3,19,21,8)])
...
#transform White and Pink to "value" and keep name in "color"
ha<-na.omit(ha)
#remove outliers
ha<-ha[ha$value<150,]
...
#no facette
p4B<-ggplot(ha,aes(x=ExacerbationRegime, y=log2(value), color=color))+geom_hline(yintercept=0,col="#02401B",linetype="dashed")+geom_boxplot(outlier.shape = NA)+ylab(expression(Log[2](pat/an)))+xlab("PEx types")+stat_compare_means(label = "..p.signif..", size=4.2,label.y=6)+ meg_stef_theme() + guides(color=guide_legend(override.aes=list(fill=NA)))+ scale_color_paper(discrete=T,palette="Noise",name="Noise\ncolor",breaks=c( 'pink','white'))+ggtitle("Pathogen behavior in distinct community backgrounds")
