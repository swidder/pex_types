...
s1A<-ggplot(p,aes(x=cclass,y=relAb,fill=Aerobicity))+geom_bar(stat="identity")+scale_fill_manual(values=c( "#E69F00" ,"purple2","#CC3366","#56B4E9","#C0C0C0"),labels=c("S4__C"="CF pathogens","S4__A"="Aerobes", "S4__F"="Fac. anaerobes", "S4__N"="Anaerobes", "S4__U"="Unknown"))+ylab("Mean rel. abundance")+xlab("Community class (DMM component)")+meg_stef_theme()+ggtitle("Mean composition of DMM comunity classes")+theme(legend.key.size=unit(16, 'points'))+ guides(fill=guide_legend(title="ASV group"))
...
save_plot("SI_figure2.pdf",sup1, ncol=1, base_height=6.5,  base_asp=1.4)
save_plot("SI_figure2.png",sup1, ncol=1, base_height=6.5,  base_asp=1.4)
...
#Aggregation by aerobicity
tmp<-otu_table(S4)
tmp<-tmp/colSums(tmp)
mS<-sample_data(S4)
mS$DMM<-paste("D",mS$DMM,sep="")
compS4<-phyloseq(otu_table(tmp), sample_data(mS), tax_table(S4))
pC1<-plot_composition(compS4, sample.sort="DMM", otu.sort="abundance", verbose=TRUE, group_by="DMM", labeller=labeller(DMM=c("1"="DMM1","2"="DMM2","3"="DMM3","4"="DMM4","5"="DMM5")))+ meg_stef_theme()+xlab("Samples") +ylab("Rel. abundance")+theme(axis.text.x=element_blank(),axis.ticks.x=element_blank())+labs(fill="ASV groups", subtitle="Count aggregation by aerobicity and pathogenicity")+ggtitle("Sample composition by DMM community classes")+scale_fill_discrete(labels=c("Anaerobes", "CF pathogens", "Fac. anaerobes","Aerobes","unknown"))

Composition by DMM community classes
Tax. aggregation at Family level

########
#Aggregation by Family (top 20 taxa)
#aggregate at Family level
compF <- aggregate_sets(EC, "Family")
#composition
tmp<-otu_table(compF)
tmp<-tmp/colSums(tmp)
#change facette titles
mS<-sample_data(compF)
mS$DMM<-paste("D",mS$DMM,sep="")
#make phyloseq object
compF<-phyloseq(otu_table(tmp), sample_data(mS), tax_table(compF))
#Identify top 20 (most abundant across taxon) and the rest to Other
top <- compF %>% taxa_sums %>% sort(decreasing=TRUE) %>% names %>% as.data.frame(labels=TRUE) %>% rename(Class=".") %>% 
  mutate(aggClass=as.factor(case_when(row_number()<=19~Class,row_number()>19~'Other')))
rownames(top)<-top[,1]
top$aggClass<-as.character(top$aggClass)

#add Other to taxonomy table
tt<-tax_table(compF)
ttt<-cbind(tt,top[rownames(tt),2])
colnames(ttt)[2]<-"Other"
compF<-phyloseq(otu_table(compF), sample_data(compF), tax_table(ttt))
#aggregate Other
compO <- aggregate_sets(compF, "Other")
#fix legend names
tS<-tax_table(compO)
rownames(tS)<-unname(sapply(tS[,1],function(x){getlast(str_split(x,"_"))}))
tO<-otu_table(compO)
rownames(tO)<-unname(sapply(rownames(tO),function(x){getlast(str_split(x,"_"))}))
compO<-phyloseq(otu_table(tO), sample_data(compF), tax_table(tS))
#plot composition with legend displaying only the top 20 most abundant taxa
pC2<-plot_composition(compO, sample.sort="DMM", otu.sort="abundance", verbose=TRUE, group_by="DMM")+ meg_stef_theme() +theme(axis.text.x=element_blank(),axis.ticks.x=element_blank()) +labs(fill="Tax", subtitle="Count aggregation at Family level")+xlab("Samples") +ylab("Rel. abundance")+ggtitle("Sample composition by DMM community classes")

#Arrange figure
#l1<-get_legend(pC1)
#l2<-get_legend(pC2)
#plot_grid(pC1+theme(legend.position="none"),l1, pC2+theme(legend.position="bottom"),l2, labels=c("A","","B",""),ncol=2, rel_widths=c(0.8,0.6))



SComp<-plot_grid(pC1+theme(legend.position="bottom"), pC2+theme(legend.position="bottom"), labels=c("E","F"),nrow=2, rel_heights=c(1,1.25))

save_plot(SComp, file="SI_figure2EF.pdf", ncol=1, base_height=6.5,  base_asp=1.3)
save_plot(SComp, file="SI_figure2EF.png", ncol=1, base_height=6.5,  base_asp=1.3)