...
p1<-df%>%ggplot(aes( x = omega2_1, y = reorder(covariates,omega2_1),fill=Pvalue)) + geom_bar(stat="identity", position="dodge")+ meg_stef_theme()+xlab(expression(paste("Partial ",eta^2)))+ylab("Covariates")+ggtitle("Effect sizes on betweenness centrality")+scale_fill_paper(name="P value",discrete = TRUE, palette = "Bi2")
...
rownames(df)<-c("PEx types","Shannon","Chao1")
df$Pvalue<-ifelse(df$p>=0.05,"n.s.",ifelse(df$p>=0.01, "*", ifelse(df$p>=0.001, "**", "***")))
p2<-df%>%ggplot(aes( x = omega2_1, y = reorder(covariates,omega2_1),fill=Pvalue)) + geom_bar(stat="identity", position="dodge")+ meg_stef_theme()+xlab(expression(paste("Partial ",eta^2)))+ylab("Covariates")+ggtitle("Effect sizes on clustering coefficient")+scale_fill_paper(name="P value",discrete = TRUE, palette = "Bi2")
...
rownames(df)<-c("PEx types","Shannon","Chao1")
df$Pvalue<-ifelse(df$p>=0.05,"n.s.",ifelse(df$p>=0.01, "*", ifelse(df$p>=0.001, "**", "***")))
p3<-df%>%ggplot(aes( x = omega2_1, y = reorder(covariates,omega2_1),fill=Pvalue)) + geom_bar(stat="identity", position="dodge")+ meg_stef_theme()+xlab(expression(paste("Partial ",eta^2)))+ylab("Covariates")+ggtitle("Effect sizes on vertex count (size)")+scale_fill_paper(name="P value",discrete = TRUE, palette = "Bi2")
...
rownames(df)<-c("PEx types","Shannon","Chao1")
df$Pvalue<-ifelse(df$p>=0.05,"n.s.",ifelse(df$p>=0.01, "*", ifelse(df$p>=0.001, "**", "***")))
p4<-df%>%ggplot(aes( x = omega2_1, y = reorder(covariates,omega2_1),fill=Pvalue)) + geom_bar(stat="identity", position="dodge")+ meg_stef_theme()+xlab(expression(paste("Partial ",eta^2)))+ylab("Covariates")+ggtitle("Effect sizes on edge count (connectivity)")+scale_fill_paper(name="P value",discrete = TRUE, palette = "Bi2")
